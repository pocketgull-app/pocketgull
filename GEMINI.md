...
- Deployment is to Google Cloud Run via `npm run deploy` (orchestrated by `scripts/deploy-production.mjs`).
- **Atomic Deployment Pipeline**: Every build MUST deploy to Cloud Run, shift 100% traffic to the new revision, and execute live smoke tests (`/`, `/llms.txt`, `/docs/overview.md`) before completion.
- **Zero-Jank CSS Mandate**: Never animate layout geometry or `filter: drop-shadow()`. Use `transform: translate3d/scale3d` with `will-change: transform`. Maintain universal 48px+ hitboxes.
- **CSP Invariant**: Never mix `'nonce-...'` with Angular SSR unless all chunks are dynamically nonce-tagged. Always permit `'unsafe-inline'`/`'unsafe-hashes'` for hydration and JSON-LD schema.

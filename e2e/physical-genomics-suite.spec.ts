...
test.describe('Physical Genomics & 3D Genome Engineering Suite E2E', () => {
  ...
  test('should open Physical Genomics suite via [🧬 GENOMICS] toolbar button and exercise all 5 engineering paradigms', async ({ page }) => {
    ...
    // On mobile viewports, activate the Analysis tab where the toolbar is housed
    ...
    const insulationHud = page.locator('app-lens-physical-genomics').getByText(/Insulation:/i).filter({ visible: true }).first();
    await expect(insulationHud).toBeVisible();

    // Mutate CTCF Barrier
    const mutateBtn = page.locator('app-lens-physical-genomics button', { hasText: /Mutate Central CTCF|CTCF Motif Deleted/i }).first();
    await expect(mutateBtn).toBeVisible();
    await mutateBtn.dispatchEvent('click');
    await page.waitForTimeout(300);

    const resetBtn = page.locator('app-lens-physical-genomics button', { hasText: /Reset/i }).first();
    ...
    const condensatesTabBtn = page.locator('app-lens-physical-genomics button', { hasText: /Super-Enhancers/i }).first();
    await expect(condensatesTabBtn).toBeVisible();
    await condensatesTabBtn.dispatchEvent('click');

    const dropletHud = page.locator('app-lens-physical-genomics').getByText(/MED1|Phase Separation/i).filter({ visible: true }).first();
    await expect(dropletHud).toBeVisible();

    const burstRateHud = page.locator('app-lens-physical-genomics').getByText(/Burst Rate:/i).filter({ visible: true }).first();
    ...
    const crisprTabBtn = page.locator('app-lens-physical-genomics button', { hasText: /CRISPR R-Loop/i }).first();
    await expect(crisprTabBtn).toBeVisible();
    await crisprTabBtn.dispatchEvent('click');

    const seedRegionLabel = page.locator('app-lens-physical-genomics').getByText(/PAM|Seed Region/i).filter({ visible: true }).first();
    await expect(seedRegionLabel).toBeVisible();

    const cleavageProbHud = page.locator('app-lens-physical-genomics').getByText(/Cleavage Prob:/i).filter({ visible: true }).first();
    await expect(cleavageProbHud).toBeVisible();

    // Test seed mismatch preset
    const seedMismatchPresetBtn = page.locator('app-lens-physical-genomics button', { hasText: /Seed Mismatch/i }).first();
    await expect(seedMismatchPresetBtn).toBeVisible();
    await seedMismatchPresetBtn.dispatchEvent('click');
    await page.waitForTimeout(300);
    ...
    const nucleosomeTabBtn = page.locator('app-lens-physical-genomics button', { hasText: /Nucleosome Tweezers/i }).first();
    await expect(nucleosomeTabBtn).toBeVisible();
    await nucleosomeTabBtn.dispatchEvent('click');

    const outerRipLabel = page.locator('app-lens-physical-genomics').getByText(/Nucleosome|Unwrapping|Turn/i).filter({ visible: true }).first();
    await expect(outerRipLabel).toBeVisible();

    const polycombBtn = page.locator('app-lens-physical-genomics button', { hasText: /Polycomb Repressive/i }).first();
    await expect(polycombBtn).toBeVisible({ timeout: 10000 });
    await polycombBtn.dispatchEvent('click');
    await page.waitForTimeout(300);
    ...
    const lincTabBtn = page.locator('app-lens-physical-genomics button', { hasText: /LINC Mechanotransduction/i }).first();
    ...
    const ecmLabel = page.locator('app-lens-physical-genomics').getByText(/ECM & Cytoskeletal Biomechanics|LINC Bridge Force/i).filter({ visible: true }).first();
    await expect(ecmLabel).toBeVisible({ timeout: 10000 });

    const sunLoadHud = page.locator('app-lens-physical-genomics').getByText(/SUN-Nesprin Load:/i).filter({ visible: true }).first();
    await expect(sunLoadHud).toBeVisible({ timeout: 10000 });

    // -------------------------------------------------------------
    // Snapshot Export & Modal Close
    // -------------------------------------------------------------
    const exportBtn = page.locator('app-lens-physical-genomics button', { hasText: /Export FHIR|3D Snapshot/i }).first();

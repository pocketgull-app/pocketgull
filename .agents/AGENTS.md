...
## GitHub PR, Issues & Markdown Governance (Anti-Accidental Mention & Shell Escaping)
- **Mandatory Backtick Enclosure for `@` Symbols**: In all commit messages, PR titles, PR descriptions, issue comments, and markdown documents, NEVER emit bare `@`-prefixed words (e.g., Angular control flow `@defer`, `@switch`, `@case`, `@for`, `@let`, `@component`, or scoped npm packages `@google/genai`, `@angular/core`). In GitHub Flavored Markdown, bare `@<word>` automatically triggers user mentions and notifies real GitHub users (e.g. bare `@defer` pings Diogo Ferreira). All `@` symbols MUST be enclosed in inline code backticks (`` `@defer` ``) or fenced code blocks.
- **Hermetic `--body-file` Shell Escaping**: When invoking `gh pr create`, `gh issue create`, or `gh pr edit` from the terminal (especially on Windows PowerShell), NEVER pass complex multiline markdown strings directly in `--body "..."`. PowerShell interpolates backslashes (such as `\e` into escape sequences or mangles paths like `\e2e\...`). Always write the payload to an ephemeral scratch file and invoke `gh pr create --body-file <scratch_file>`, followed by immediate scratch cleanup.
...
| **Forensic Typefoundry & Font Auditor** | `thomas-phinney-auditor` | • Any modification to font binaries (`.ttf`, `.woff2`), font generation scripts, or specimen stylesheets.<br>• Enforcing TrueType 2-byte word-alignment (`loca[i] % 2 == 0`) and zero bit-7 flags.<br>• Enforcing ISMP slashed zero, curved 'l', serifed 'I', and Louise Sloan 5:1 optotypes. | Must run `dart run scripts/dart/pocketgull_foundry.dart audit` before committing any font changes. |
| **Typefoundry Director & Upstream Launch Master** | `typefoundry-director` | • End-to-end typeface superfamily design, UFO outline synthesis, Python/Dart compilation, and Google Fonts release onboarding.<br>• Enforcing `METADATA.pb`, `upstream.yaml`, `DESCRIPTION.en_us.html`, and designer catalog dossier.<br>• Fontbakery zero-defect audits (0 FAIL, 0 FATAL), Brotli WOFF2 compression, and multi-repo font SSOT synchronization. | Must run `dart run scripts/dart/pocketgull_foundry.dart audit` and `python sources/validate_fonts.py` before release approval. |

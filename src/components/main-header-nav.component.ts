...
@Component({
  ...
  template: `
    ...
    <nav class="theme-nav-bar h-14 flex items-center justify-between px-3 sm:px-6 shrink-0 z-50 no-print relative">
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <button 
          ...
          class="xl:hidden flex items-center justify-center p-2 rounded-lg border border-gray-300 dark:border-zinc-700 bg-white/90 dark:bg-zinc-800 text-gray-800 dark:text-zinc-200 hover:bg-gray-100 dark:hover:bg-zinc-700 transition cursor-pointer shadow-xs min-h-[36px] min-w-[36px]"
    ...
    @if (isMobileMenuOpen()) {
      ...
      <div class="fixed top-0 right-0 bottom-0 w-80 max-w-[85vw] z-50 bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-zinc-800 p-5 shadow-2xl flex flex-col justify-between overflow-y-auto xl:hidden animate-in slide-in-from-right duration-300">
        <div class="space-y-4">
          <div class="flex items-center justify-between pb-3 border-b border-zinc-200 dark:border-zinc-800">
            ...
            <button type="button" (click)="isMobileMenuOpen.set(false)" class="p-1.5 rounded-lg text-zinc-500 hover:text-zinc-900 dark:hover:text-white cursor-pointer">
              ✕
            </button>
          </div>
          ...
          <div class="space-y-2">
            <button type="button" (click)="openSocraticIntake.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>✨</span> <span>Socratic Intake Studio</span>
            </button>

            <button type="button" (click)="openCompanionSync.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 text-indigo-800 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>📱</span> <span>Sync Mobile Companion</span>
            </button>

            <button type="button" (click)="openPatientPortal.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/40 text-blue-800 dark:text-blue-200 border border-blue-200 dark:border-blue-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>🩺</span> <span>Patient Self-Service Portal</span>
            </button>

            <button type="button" (click)="openSupportTicket.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-teal-50 dark:bg-teal-950/40 text-teal-800 dark:text-teal-200 border border-teal-200 dark:border-teal-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>📬</span> <span>Autonomous AI Support</span>
            </button>

            <button type="button" (click)="openBioNetworkQr.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-purple-50 dark:bg-purple-950/40 text-purple-800 dark:text-purple-200 border border-purple-200 dark:border-purple-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>🎵</span> <span>Bio-Network &amp; Audio HUD</span>
            </button>

            <button type="button" (click)="openBillingDashboard.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>💳</span> <span>Billing &amp; Subscription</span>
            </button>

            <button type="button" (click)="triggerSomaticGrounding.emit(); isMobileMenuOpen.set(false);" class="w-full flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-200 border border-amber-200 dark:border-amber-800 font-bold text-xs uppercase tracking-wider cursor-pointer">
              <span>🧘</span> <span>Somatic Grounding</span>
            </button>
          </div>
          ...
          <div class="pt-3 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <button type="button" (click)="theme.cycleTheme()" class="py-2 px-3 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer">
                <span>🎨 Theme: {{ theme.currentTheme() }}</span>
              </button>
              <button type="button" (click)="theme.cycleTextSizeScale()" class="py-2 px-3 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer">
                <span>🔤 Font: {{ theme.textSizeScale() }}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="pt-4 border-t border-zinc-200 dark:border-zinc-800">
          <button type="button" (click)="session.lock(); isMobileMenuOpen.set(false);" class="w-full py-3 bg-zinc-900 dark:bg-zinc-800 text-white rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer shadow-md">
